...
public class QuestionManager : MonoBehaviour
{
    ...
    public int qipointBonus = 10; // Nombre de points QI à gagner pour une question QI correctement répondu
    public int qipointMalus = 10; // Nombre de points QI à perdre pour une question QI mal répondu
    ...
    void Start()
    {
        ...
        questionsList.Add(
            new Question("Quel est l'inventeur de la théorie de la relativité ?", "Albert Einstein", "qi"));
        questionsList.Add(new Question("Quel est le nom du président français actuel ?", "Emmanuel Macron", "argent"));
        questionsList.Add(new Question("Quelle est la plus haute montagne du monde ?", "Everest", "argent"));
...
>>>>>>> Stashed changes
    ...
    IEnumerator WaitForAnswer(Question question)
    {
        ...
        while (true)
        {
            ...
            // Vérifier si le joueur a sélectionné une réponse
            if (Input.GetKeyDown(KeyCode.Alpha1))
            {
                if (question.CheckAnswer(question.options[0]))
                {
                    // Attribuer des points ou de l'argent en fonction du type de question
                    if (question.type == "qiBonus")
                    {
                        score += qipointBonus;
                    }
            ...
            {
                if (question.CheckAnswer(question.options[1]))
                {
                    // Attribuer des points ou de l'argent en fonction du type de question
                    if (question.type == "qi")
                    {
...
public class Question
{
    ...
    public bool CheckAnswer(string answer=null)
    {
        ...
        GameObject inputFieldObject = GameObject.Find("PlayerController");
